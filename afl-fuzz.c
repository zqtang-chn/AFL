...
EXP_ST u8 crash_deduplication, // crash 去重模式
...
static void read_crashs(char **argv)
{
  struct dirent **nl;
  s32 nl_cnt;
  u32 i;

  u8 *crashDir = "/tmp/crash";
  nl_cnt = scandir(crashDir, &nl, NULL, alphasort);
  if (nl_cnt < 0)
    return;

  for (i = 0; i < nl_cnt; i++)
  {
    u8 *fn = alloc_printf("%s/%s", crashDir, nl[i]->d_name);
    struct stat st;
    if (lstat(fn, &st) || access(fn, R_OK))
      continue;
    if (0 == strcmp("..", nl[i]->d_name) || 0 == strcmp(".", nl[i]->d_name))
      continue;
    FILE *fp = fopen(fn, "rb");
    if (NULL == fp)
      continue;
    char *buffer = malloc(st.st_size * sizeof(char));
    if (st.st_size != fread(buffer, sizeof(char), st.st_size, fp))
    {
      fclose(fp);
      continue;
    }
    fclose(fp);
    common_fuzz_stuff(argv, buffer, st.st_size);
    remove(fn);
  }
  if (nl)
    free(nl);
...
static u8 fuzz_one(char **argv)
{
  ...
  if (crash_deduplication)
  { // crash 去重{
    stage_short = "crash dedup";
    stage_max = 0;
    stage_name = "crash deduplication";
    read_crashs(argv);
    FATAL("Crash de-duplication completed.");
  }
...
int main(int argc, char **argv)
{
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dRnCB:S:M:x:Q")) > 0)
